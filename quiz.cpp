...
class quizFields{
    int choice,quizMarks=0,questionNumber=1;
    public:
        quizFields(){
            while(true){
                cout << "THERE ARE THREE QUIZ FIELDS THAT ARE AVAILABLE PLEASE PICK YOURS." << endl;
                cout << "Enter 1 for space quiz, 2 for general knowledge quiz and 3 for map quiz or anything else to exit:";
                cin >> choice;
                cout << endl << endl;
                if(choice == 1){
                    spaceQuiz();
...
void quizFields::spaceQuiz(){
    ...
    //RESULT PRINTING AND ATTRIBUTES RESET
    cout << "OKAY THATS ALL THANK YOU FOR PARTICIPATING IN THIS SPACE QUIZ." << endl << endl;
    cout << "YOU SCORED " << quizMarks << " OUT OF 10" << endl << endl;
    quizMarks = 0;
    questionNumber = 1;
}
